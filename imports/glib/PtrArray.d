...
public class PtrArray
{
	...
	{
		return g_ptr_array_is_null_terminated(gPtrArray) != 0;
	}
	 ...
	 *     copyFunc = a copy function used to copy every element in the
	 *         array or %NULL.
	 *     copyFuncUserData = user data passed to @copy_func, or %NULL
	 *     elementFreeFunc = a function to free elements on @array
	 *         destruction or %NULL
	 *
	 ...
	 * Throws: ConstructionException GTK+ fails to create the object.
	 */
	public this(void*[] data, GCopyFunc copyFunc, void* copyFuncUserData, GDestroyNotify elementFreeFunc)
	{
		auto __p = g_ptr_array_new_from_array(data.ptr, cast(size_t)data.length, copyFunc, copyFuncUserData, elementFreeFunc);
	 ...
	 *     elementFreeFunc = A function to free elements with
	 *         destroy @array or %NULL
	 *     nullTerminated = whether to make the array as %NULL terminated.
	 *
	 * Returns: A new #GPtrArray
	 *
	 * Since: 2.74
	 ...
	 * Throws: ConstructionException GTK+ fails to create the object.
	 */
	public this(uint reservedSize, GDestroyNotify elementFreeFunc, bool nullTerminated)
	{
		auto __p = g_ptr_array_new_null_terminated(reservedSize, elementFreeFunc, nullTerminated);

		if(__p is null)
		{
			throw new ConstructionException("null returned by new_null_terminated");
		}
	 ...
	 * Throws: ConstructionException GTK+ fails to create the object.
	 */
	public this(void*[] data, GDestroyNotify elementFreeFunc)
	{
		auto __p = g_ptr_array_new_take(data.ptr, cast(size_t)data.length, elementFreeFunc);
	 ...
	 *
	 * Params:
	 *     compareFunc = a #GCompareFunc comparison function
	 *
	 * Since: 2.76
	 */
	public void sortValues(GCompareFunc compareFunc)
	{
		g_ptr_array_sort_values(gPtrArray, compareFunc);
	}
	 ...
	 *
	 * Params:
	 *     compareFunc = a #GCompareDataFunc comparison function
	 *     userData = data to pass to @compare_func
	 *
	 * Since: 2.76
	 */
	public void sortValuesWithData(GCompareDataFunc compareFunc, void* userData)
	{
		g_ptr_array_sort_values_with_data(gPtrArray, compareFunc, userData);
	}
