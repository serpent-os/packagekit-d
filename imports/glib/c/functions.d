...
__gshared extern(C)
{
	...
	GArray* g_array_new_take(void* data, size_t len, int clear, size_t elementSize);
	GArray* g_array_new_take_zero_terminated(void* data, int clear, size_t elementSize);
	...
	void* g_bytes_get_region(GBytes* bytes, size_t elementSize, size_t offset, size_t nElements);
	...
	GPtrArray* g_hash_table_get_keys_as_ptr_array(GHashTable* hashTable);
	GList* g_hash_table_get_values(GHashTable* hashTable);
	GPtrArray* g_hash_table_get_values_as_ptr_array(GHashTable* hashTable);
	...
	GHashTable* g_hash_table_new_similar(GHashTable* otherHashTable);
	...
	GPtrArray* g_hash_table_steal_all_keys(GHashTable* hashTable);
	GPtrArray* g_hash_table_steal_all_values(GHashTable* hashTable);
	...
	int g_path_buf_pop(GPathBuf* buf);
	GPathBuf* g_path_buf_push(GPathBuf* buf, char* path);
	int g_path_buf_set_extension(GPathBuf* buf, char* extension);
	int g_path_buf_set_filename(GPathBuf* buf, char* fileName);
	...
	int g_pattern_spec_match(GPatternSpec* pspec, size_t stringLength, const(char)* string_, const(char)* stringReversed);
	int g_pattern_spec_match_string(GPatternSpec* pspec, const(char)* string_);
	...
	int g_ptr_array_is_null_terminated(GPtrArray* array);
	GPtrArray* g_ptr_array_new();
	GPtrArray* g_ptr_array_new_from_array(void** data, size_t len, GCopyFunc copyFunc, void* copyFuncUserData, GDestroyNotify elementFreeFunc);
	GPtrArray* g_ptr_array_new_from_null_terminated_array(void** data, GCopyFunc copyFunc, void* copyFuncUserData, GDestroyNotify elementFreeFunc);
	GPtrArray* g_ptr_array_new_full(uint reservedSize, GDestroyNotify elementFreeFunc);
	GPtrArray* g_ptr_array_new_null_terminated(uint reservedSize, GDestroyNotify elementFreeFunc, int nullTerminated);
	GPtrArray* g_ptr_array_new_take(void** data, size_t len, GDestroyNotify elementFreeFunc);
	GPtrArray* g_ptr_array_new_take_null_terminated(void** data, GDestroyNotify elementFreeFunc);
	...
	void g_ptr_array_sort_values(GPtrArray* array, GCompareFunc compareFunc);
	void g_ptr_array_sort_values_with_data(GPtrArray* array, GCompareDataFunc compareFunc, void* userData);
	...
	char* g_regex_escape_string(const(char)* string_, int length);
	...
	char* g_string_free_and_steal(GString* string_);
	...
	void g_test_case_free(GTestCase* testCase);
	...
	GThreadPool* g_thread_pool_new_full(GFunc func, void* userData, GDestroyNotify itemFreeFunc, int maxThreads, int exclusive, GError** err);
	...
	GModule* g_module_open_full(const(char)* fileName, GModuleFlags flags, GError** err);
	...
	int g_spawn_check_exit_status(int waitStatus, GError** err);
	void g_spawn_close_pid(GPid pid);
	int g_spawn_command_line_async(char* commandLine, GError** err);
	int g_spawn_command_line_sync(char* commandLine, char** standardOutput, char** standardError, int* waitStatus, GError** err);
	GQuark g_spawn_error_quark();
	GQuark g_spawn_exit_error_quark();
	int g_spawn_sync(char* workingDirectory, char** argv, char** envp, GSpawnFlags flags, GSpawnChildSetupFunc childSetup, void* userData, char** standardOutput, char** standardError, int* waitStatus, GError** err);
