...
public class ListStore : ObjectG, ListModelIF
{
	 ...
	 *     item = an item
	 *     equalFunc = A custom equality check function
	 *     userData = user data for @equal_func
	 *     position = the first position of @item, if it was found.
	 *
	 * Returns: Whether @store contains @item. If it was found, @position will be
	 *     set to the position where @item occurred for the first time.
	 ...
	 * Since: 2.74
	 */
	public bool findWithEqualFuncFull(ObjectG item, GEqualFuncFull equalFunc, void* userData, out uint position)
	{
		return g_list_store_find_with_equal_func_full(gListStore, (item is null) ? null : item.getObjectGStruct(), equalFunc, userData, &position) != 0;
	}
