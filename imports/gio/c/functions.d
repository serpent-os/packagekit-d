...
__gshared extern(C)
{
	...
	void g_app_info_get_default_for_type_async(const(char)* contentType, int mustSupportUris, GCancellable* cancellable, GAsyncReadyCallback callback, void* userData);
	GAppInfo* g_app_info_get_default_for_type_finish(GAsyncResult* result, GError** err);
	GAppInfo* g_app_info_get_default_for_uri_scheme(const(char)* uriScheme);
	void g_app_info_get_default_for_uri_scheme_async(const(char)* uriScheme, GCancellable* cancellable, GAsyncReadyCallback callback, void* userData);
	GAppInfo* g_app_info_get_default_for_uri_scheme_finish(GAsyncResult* result, GError** err);
	...
	int g_debug_controller_get_debug_enabled(GDebugController* self);
	void g_debug_controller_set_debug_enabled(GDebugController* self, int debugEnabled);

	// gio.DebugControllerDBus

	GType g_debug_controller_dbus_get_type();
	GDebugControllerDBus* g_debug_controller_dbus_new(GDBusConnection* connection, GCancellable* cancellable, GError** err);
	...
	void g_file_new_tmp_async(char* tmpl, int ioPriority, GCancellable* cancellable, GAsyncReadyCallback callback, void* userData);
	void g_file_new_tmp_dir_async(char* tmpl, int ioPriority, GCancellable* cancellable, GAsyncReadyCallback callback, void* userData);
	GFile* g_file_new_tmp_dir_finish(GAsyncResult* result, GError** err);
	GFile* g_file_new_tmp_finish(GAsyncResult* result, GFileIOStream** iostream, GError** err);
	...
	void g_file_make_symbolic_link_async(GFile* file, char* symlinkValue, int ioPriority, GCancellable* cancellable, GAsyncReadyCallback callback, void* userData);
	int g_file_make_symbolic_link_finish(GFile* file, GAsyncResult* result, GError** err);
	...
	void g_file_move_async(GFile* source, GFile* destination, GFileCopyFlags flags, int ioPriority, GCancellable* cancellable, GFileProgressCallback progressCallback, void* progressCallbackData, GAsyncReadyCallback callback, void* userData);
	int g_file_move_finish(GFile* file, GAsyncResult* result, GError** err);
	...
	void g_file_info_set_creation_date_time(GFileInfo* info, GDateTime* creationTime);
	...
	int g_list_store_find_with_equal_func_full(GListStore* store, void* item, GEqualFuncFull equalFunc, void* userData, uint* position);
	...
	int g_power_profile_monitor_get_power_saver_enabled(GPowerProfileMonitor* monitor);
	...
	void g_task_set_static_name(GTask* task, const(char)* name);
	...
	GTlsCertificate* g_tls_certificate_new_from_file_with_password(char* file, const(char)* password, GError** err);
	...
	GTlsCertificate* g_tls_certificate_new_from_pkcs12(ubyte* data, size_t length, const(char)* password, GError** err);
	...
	char* g_content_type_guess(char* filename, char* data, size_t dataSize, int* resultUncertain);
